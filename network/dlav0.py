...
class DLASeg(nn.Module):
    def __init__(self, base_name, heads,
        ...
        self.base = globals()[base_name](
          pretrained=pretrained, return_levels=True)   # globals 函数返回一个全局变量的字典，包括所有导入的变量
    ...
    def forward(self, x):
        ...
        # cls = torch.sigmoid(ret['hm'])
        # wh = ret['wh']
        # reg = ret['reg']
        # pred = {"cls": cls, "wh": wh, "reg": reg}
        # fmap_max = nn.MaxPool2d(kernel_size=3, stride=1, padding=1)(pred['cls'])
        # keep = (pred['cls'] - fmap_max).float() + 1e-9
        # keep = nn.ReLU()(keep)
        # keep = keep * 1e9
        # result = pred['cls'] * keep
        # ret = [result, pred['reg'], pred['wh']]  ## change dict to list
        ...
        # wh = out[-1][1]
        # reg = out[-1][2]
        # hm = hm.reshape(1, 25, 384//4, 768//4)
        # heat = _nms(hm)
        # output = []
        # output.append(heat)
        # output.append(wh)
...
# def _nms(heat, kernel=3):   #检测当前热点的值是否比周围的八个近邻点(八方位)都大(或者等于)
#     pad = (kernel - 1) // 2
#
...
if __name__ == "__main__":
    test_tensor = torch.rand([1, 3, 384, 768]).cuda()
    model = DLASeg('dla{}'.format(34), {'hm': 25, 'wh':2 ,'reg':2},
                 pretrained=True,
                 down_ratio=4,
                 head_conv=256)
    model.cuda()
